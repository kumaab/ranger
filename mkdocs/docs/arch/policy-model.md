...
:   Describes a *type* of service: which resource levels exist (for Hive: `database`, `table`,
    ...
    and loads them into the database on first start. You can add your own with the REST API; see
    [Custom plugins](../plugins/custom-plugin.md). The full field list is in
...
User-attribute expressions need the user store enricher; see [ABAC](../features/abac.md).
...
```mermaid
flowchart TD
  REQ[Access request] --> SU{Super user or<br/>super group?}
  SU -->|yes| ALLOW1[Allow]
  SU -->|no| ZONE{Resource in a<br/>security zone?}
  ZONE -->|yes| ZP[Use that zone's policies]
  ZONE -->|no| DP[Use default zone policies]
  ZP --> TAG
  DP --> TAG
  TAG[Evaluate tag-based policies<br/>for each tag on the resource] --> RES
  RES[Evaluate resource-based policies<br/>highest priority first] --> DET{Access<br/>determined?}
  DET -->|denied| DENY[Deny]
  DET -->|allowed| ALLOW2[Allow]
  DET -->|undetermined| FB{Plugin supports<br/>fallback?}
  FB -->|yes: HDFS, YARN| NATIVE[Defer to native ACLs]
  FB -->|no| DENY
```
...
1. **Super users.** If the user is listed in `ranger.plugin.<type>.super.users` or belongs to a
   group in `ranger.plugin.<type>.super.groups`, access is allowed immediately with the reason
   `superuser`.
...
* [Resource-based policies](../features/policies/resource-policies.md): creating policies in the UI and REST API.
* [Tag-based policies](../features/policies/tag-based-policies.md).
* [Row filters and column masking](../features/policies/row-filter-column-masking.md).
* [Policy conditions](../features/policies/policy-conditions.md): IP ranges, time of day, scripts, and other conditions.
* [Roles](../features/roles.md) and [Attribute-based access control](../features/abac.md).
