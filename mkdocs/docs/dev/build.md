...
`target/version` into `dev-support/ranger-docker/dist/`, where the service Dockerfiles expect them. Your
`~/.m2` is mounted into the container, so dependency downloads are cached between runs.
...
Environment variables read by `ranger-build.sh` (defaults are set in `dev-support/ranger-docker/.env`):
...
| `SKIPTESTS` | `true` | Set to `false` to run unit tests in the container. |
| `JAVA_OPTS` | JDK 17 `--add-opens` / `--add-exports` options | Appended to `MAVEN_OPTS`. |

After cloning, the script applies any patches placed in `dev-support/ranger-docker/patches/` with `git apply`.

### Regular build for the Docker setup

To use a build made with Maven on your machine, copy the archives into `dist/` yourself:
...
Either way, continue with [Running Ranger with Docker](../getting-started/docker.md#build-from-source) to build
the service images from `dist/` and start the containers.
...
| Stale `dev-support/ranger-docker/dist` after a rebuild | Copy `target/ranger-*` and `target/version` into `dist/` again, then rebuild the affected images with `up -d --no-deps --force-recreate --build <service>`. |
