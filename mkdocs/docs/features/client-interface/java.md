...
`RangerRESTClient` sends `Authorization: Bearer <token>` when a token supplier, a
`java.util.function.Supplier<String>`, is set. The supplier is called for every request, so it can return
a refreshed token at any time. `DefaultTokenSupplier` reads the token from an environment variable, a file
(re-read when it changes) or a Hadoop credential provider (re-read at most once a minute), selected by
`<prefix>.jwt.source` = `env` | `file` | `cred`:
...
restClient.setTokenSupplier(new DefaultTokenSupplier("ranger.client", conf));

RangerClient ranger = new RangerClient(restClient);
```

Pass your own `Supplier<String>` to `setTokenSupplier` to obtain and refresh tokens from an identity
provider, for example `restClient.setTokenSupplier(() -> tokenCache.currentToken())`. Plugins pick a
supplier from configuration instead; see
[Plugin architecture](../../arch/plugin-architecture.md#security-xml).
