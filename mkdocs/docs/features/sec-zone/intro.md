...
Security zones were introduced in Ranger 2.0.0 (RANGER-2232). This page covers the model, the
...
:   `tagServices` lists tag services whose tag-based policies apply inside the zone. Tag policies are
    evaluated in the same zone as the accessed resource (RANGER-2343), so a zone without a tag service
...
- [Users, groups and roles](../../services/admin/users-groups-roles.md) — Admin user roles referenced in the table above.
