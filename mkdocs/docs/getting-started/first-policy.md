...
two more kinds of policy in action. Everything runs in the Docker environment from
[Run Ranger with Docker](docker.md), which starts Hive with the Ranger Hive plugin already enabled and
a Ranger service named `dev_hive` already created.

Allow about 20 minutes. No Hive or Ranger experience is needed.

## Before you start

Start Ranger Admin, the audit pipeline, Hadoop and Hive:
...
(If you have not built Ranger yet, run the build step from [Run Ranger with Docker](docker.md#compose)
first. `ENABLED_RANGER_SERVICES="hadoop,hive" ./ranger_in_docker up` from the repository root also
builds and starts Ranger, Hadoop and Hive, but not the audit pipeline used in step 6.)
