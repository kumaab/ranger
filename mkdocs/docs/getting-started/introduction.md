...
- **Open to any application.** Embed the policy engine as a library, or call the
  [Ranger PDP](../services/pdp/service.md) over REST; see
...
|---|---|---|
| [Polaris](../plugins/polaris.md) | Apache Polaris catalog service | catalog, namespace, table, policy, principal |
...
| [Ozone](../plugins/ozone.md) | Ozone Manager | volume, bucket, key |
| [Nested structure](../plugins/nestedstructure.md) | Any application that embeds the plugin | schema, field |
| [Kudu](../plugins/kudu.md) | Kudu master | database, table, column |
| [Schema Registry](../plugins/schema-registry.md) | Schema Registry server | schema group, schema metadata, branch, version, serde |
| [Presto](../plugins/presto.md) | Presto coordinator | catalog, schema, table, column |
| [Elasticsearch](../plugins/elasticsearch.md) | Elasticsearch node | index |
| [Kylin](../plugins/kylin.md) | Kylin server | project |
| [Sqoop](../plugins/sqoop.md) | Sqoop 2 server | connector, link, job |
| [NiFi Registry](../plugins/nifi-registry.md) | NiFi Registry | registry resource |
| [NiFi](../plugins/nifi.md) | NiFi | NiFi resource |
| [Atlas](../plugins/atlas.md) | Atlas server | type, entity, classification, relationship |
| [Kafka](../plugins/kafka.md) | Broker (authorizer) | topic, consumer group, cluster, transactional id |
| [Solr](../plugins/solr.md) | Solr node | collection, schema, config, admin |
| [KMS](../plugins/kms.md) | Ranger KMS | encryption key |
| [YARN](../plugins/yarn.md) | ResourceManager | queue |
| [Knox](../plugins/knox.md) | Knox gateway | topology, service |
| [Storm](../plugins/storm.md) | Nimbus | topology |
| [HBase](../plugins/hbase.md) | Master and RegionServers (coprocessor) | table, column family, column |
| [Hive](../plugins/hive.md) | HiveServer2, optionally the Metastore | database, table, column, UDF, URL |
| [HDFS](../plugins/hdfs.md) | NameNode | path |

Service definitions for `abfs`, `wasb`, `tag` and `gds` are also shipped; see
[Other service definitions](../plugins/other-service-definitions.md). The complete list is in
...
embedded plugin or through the PDP; see [Writing a custom plugin](../plugins/custom-plugin.md).
...
:   Every plugin decision produces an access audit event (who, what resource, which access type, the
    ...
    OpenSearch, Elasticsearch, HDFS or the Ranger audit server. See
...
:   Policies grant access to users, groups and roles. Ranger UserSync imports users and groups from
    ...
    roles. See [Users, groups and roles](../services/admin/users-groups-roles.md) and
...
:   Classifications such as `PII` or `CONFIDENTIAL` attached to resources, usually synchronized from
    ...
    regardless of which table or service they live in. See
...
```mermaid
flowchart LR
  subgraph admin["Ranger Admin"]
    UI[Web UI / REST API] --> DB[(Policy database)]
  end
  subgraph component["Protected service, e.g. Trino coordinator"]
    P[Ranger plugin] --> C[(Local policy cache)]
  end
  DB -- "policies (pull, every 30 s by default)" --> P
  U[User request] --> component
  P -- "allow / deny" --> component
  P -- "audit events (async)" --> A[(Audit store)]
...
exposes the same policy engine over REST; see [Ranger PDP](../services/pdp/service.md).
...
[Policy database](../services/admin/database.md)
...
[Audit server](../services/audit-server/service.md) and [audit stores](../services/audit/index.md)
:   Ingest, dispatch and store access audits (Solr, OpenSearch, Elasticsearch, HDFS, …).
...
- [Run Ranger with Docker](docker.md) — a complete stack, with protected services, in a few commands.
