...
`ranger.plugin.audit.filters` to the service if needed. See [audit filters](../services/audit/audit-filters.md).

## Try it with Docker

`dev-support/ranger-docker` uses Elasticsearch/OpenSearch only as an *audit store*; there is no
Ranger-protected Elasticsearch container. To experiment, start Ranger Admin with Docker (see
[Running Ranger with Docker](../getting-started/docker.md)) and point an Elasticsearch cluster that can reach
it at `http://<docker-host>:6080` using the configuration above.

## Further reading

- [Plugin architecture](../arch/plugin-architecture.md), [resource-based policies](../features/policies/resource-policies.md)
- [Audit stores](../services/audit/audit-stores.md), which covers Elasticsearch as the Ranger audit index
