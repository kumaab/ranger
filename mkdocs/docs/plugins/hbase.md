...
- Opening a region of a system table (`hbase:meta`, `-ROOT-`, `.META.`, `hbase:acl`, `hbase:namespace`)
  ...
  `admin`.
- Read access to the system tables `hbase:meta`, `-ROOT-`, `.META.` and `hbase:namespace` is always
  allowed without consulting policies and is not audited, because HBase clients need it to locate
  regions. `hbase:acl` is the exception: only the HBase system user and superusers read it without a
  policy check; for every other user a read of `hbase:acl` is authorized against Ranger policies and
  audited like a read of any other table.
...
`docker-compose.ranger-hbase.yml` starts a `ranger-hbase` container (Master on 16000/16010, RegionServer on
16020/16030) on top of the `ranger-hadoop` and `ranger-zk` containers.
...
Ranger service `dev_hbase`, which the plugin in the container enforces.

To verify:

- `docker logs ranger` shows `dev_hbase service created` (or `dev_hbase service already exists` on a restart).
- In Ranger Admin at `http://localhost:6080` (`admin` / `rangerR0cks!`), the service appears in the
  service manager and **Audit → Plugin Status** lists `dev_hbase` once the plugin has downloaded its policies.
- The HBase Master UI answers at `http://localhost:16010`.
