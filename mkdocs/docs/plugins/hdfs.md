...
(NameNode, DataNode, ResourceManager, NodeManager) with the HDFS and YARN plugins active. The NameNode
enforces the Ranger service `dev_hdfs` and pulls policies from `http://ranger.rangernw:6080`.
...
The NameNode RPC port 9000 and the ResourceManager UI port 8088 are published on the host. See
