...
- The plugin jars. The Ranger build produces `ranger-<version>-hive-plugin.tar.gz` (see
  Build from source); its `lib/` directory holds the plugin shim jars and the
...
the query runs and cannot be bypassed by views or CTEs.
...
- `CREATE ROLE`, `DROP ROLE`, `GRANT ROLE`, `REVOKE ROLE` and `SHOW ROLES` are handled by the plugin and
  mapped to Ranger roles. `SHOW GRANT` is supported for Hive resources, not for
  principals.
...
Apache Impala does not have a service definition of its own in Ranger: it is authorized through a Ranger
service of type `hive`, so Impala policies are written against the resources and access types described on
this page. The authorizer ships with Apache Impala, not in the `apache/ranger` repository.
According to
[Impala's authorization documentation](https://impala.apache.org/docs/build/html/topics/impala_authorization.html),
it is enabled on the `catalogd` daemon and on every `impalad` daemon with the startup flags
`-authorization_provider=ranger`, `-ranger_service_type=hive`, `-ranger_app_id` and `-server_name`, and it
reads `ranger-hive-security.xml` from Impala's configuration directory. Refer to that documentation for the
supported privileges and limitations.
...
`docker-compose.ranger-hive.yml` starts a `ranger-hive` container with HiveServer2 (port 10000) and the
metastore (port 9083) on top of the `ranger-hadoop` container.
...
Ranger service `dev_hive`, which the plugin in the container enforces.

To verify:

- `docker logs ranger` shows `dev_hive service created` (or `dev_hive service already exists` on a restart).
- In Ranger Admin at `http://localhost:6080` (`admin` / `rangerR0cks!`), the service appears in the
  service manager and **Audit → Plugin Status** lists `dev_hive` once the plugin has downloaded its policies.
- Connect with `beeline -u jdbc:hive2://localhost:10000` from inside the container
  (`docker exec -it ranger-hive bash`), run a statement and look for it under **Audit → Access**.

The first policy tutorial walks through a complete example.
