...
(`PLAINTEXT`) listener, where every client is the `ANONYMOUS` user.
...
and `__CruiseControlMetrics` topics and of `atlas` and `rangertagsync` on consumer groups.
...
adds a `ranger-kafka` container (hostname `ranger-kafka.rangernw`, port `9092`) to the Ranger environment. It
depends on the `ranger` and `ranger-zk` containers.
...
docker compose --profile ${AUDIT_DESTINATIONS} -f docker-compose.ranger.yml -f docker-compose.ranger-audit-service.yml -f docker-compose.ranger-kafka.yml up -d
```

The README has no separate command for Kafka; the line above adds `docker-compose.ranger-kafka.yml` to the
same base files the README uses for every other service.

When Ranger Admin becomes ready, its bootstrap script `scripts/admin/create-ranger-services.py` creates the
Ranger service `dev_kafka`, which the plugin in the container enforces.

To verify:

- `docker logs ranger` shows `dev_kafka service created` (or `dev_kafka service already exists` on a restart).
- In Ranger Admin at `http://localhost:6080` (`admin` / `rangerR0cks!`), the service appears in the
  service manager and **Audit → Plugin Status** lists `dev_kafka` once the plugin has downloaded its policies.
- `docker logs ranger-kafka` shows the broker starting; broker access events appear under **Audit → Access**.

The `dev_kafka` service is created with a few starter policies (for example `rangertagsync` on topic
`ATLAS_ENTITIES`), and the broker sends audits to the Ranger audit server. With `KERBEROS_ENABLED=true` (the
default in `.env`) the broker uses a `SASL_PLAINTEXT` listener with GSSAPI, `super.users=User:kafka`, and the
service restricts policy download to the `kafka` user. With Kerberos disabled the environment leaves
`authorizer.class.name` commented out, because every client would be `ANONYMOUS`.
