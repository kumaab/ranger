...
`dev-support/ranger-docker/docker-compose.ranger-kms.yml` starts a `ranger-kms` container (port 9292).
Its plugin enforces the Ranger service `dev_kms` and pulls policies from `http://ranger.rangernw:6080`.
...
- [HSM and key stores](../services/kms/hsm-and-key-stores.md)
- [Users, groups and roles](../services/admin/users-groups-roles.md): the KeyAdmin and KeyAuditor roles
