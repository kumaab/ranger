...
`dev-support/ranger-docker/docker-compose.ranger-knox.yml` starts a `ranger-knox` container (gateway
on port 8443) with the plugin active and a `sandbox` topology that references `XASecurePDPKnox`. The
gateway enforces the Ranger service `dev_knox`.
...
Requests to `https://localhost:8443/gateway/sandbox/webhdfs/v1/...` are then authorized by the
policies of the `dev_knox` service. See [Running Ranger with Docker](../getting-started/docker.md) for
