...
`ranger.plugin.audit.filters` to the service configuration if you need to reduce volume. See
[audit filters](../services/audit/audit-filters.md).

## Try it with Docker

There is no Kylin container in `dev-support/ranger-docker`. To experiment, start Ranger Admin with Docker
(see [Running Ranger with Docker](../getting-started/docker.md)) and point a Kylin server that can reach it
at `http://<docker-host>:6080` using the configuration above.
