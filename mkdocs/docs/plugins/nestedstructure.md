...
- The library jar from the Ranger build (`plugin-nestedstructure/target/ranger-nestedstructure-plugin-<version>.jar`,
  see [Build](../dev/build.md)) and its dependencies on the application classpath.
...
There is no dedicated compose file; run your application against a Ranger Admin started with
...
- [Authorization API](../dev/authz-api.md) — the newer API for embedding Ranger in an application
