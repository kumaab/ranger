...
`dev-support/ranger-docker` has no NiFi compose file. Run Ranger with Docker (see
[Run Ranger with Docker](../getting-started/docker.md)), start a NiFi container on the `rangernw` network
and apply the configuration above.
