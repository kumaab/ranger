...
allowed-request filters in the service configuration if you need them. See
...
which starts three containers: `ozone-scm` and `ozone-datanode` on the `apache/ozone-runner` image, and
`ozone-om` on the `ranger-ozone` image built from it (`Dockerfile.ranger-ozone`). The OM image sets `OZONE_MANAGER_CLASSPATH` to the plugin directory mounted from `dist/`, and
the environment's Ozone configuration turns on Kerberos, `ozone.acl.enabled=true` and the Ranger authorizer
class.
...
./scripts/ozone/ozone-plugin-docker-setup.sh   # unpacks the plugin archive into dist/
...
The OM enforces the `dev_ozone` service (`ozone.om.http-address=http://om:9874`), which the environment
creates in Ranger Admin with `policy.download.auth.users=om`, and sends audits to the Ranger audit server. Ports published on the host: OM HTTP
`9874`, OM RPC `9862`, SCM HTTP `9876`, SCM RPC `9860`.
[`ozone-action-matcher.md`](https://github.com/apache/ranger/blob/master/dev-support/ranger-docker/ozone-action-matcher.md)
in the same directory explains how to try the action matcher in this environment. See
[Running Ranger with Docker](../getting-started/docker.md) for the rest of the environment.

## Further reading

- [Plugin architecture](../arch/plugin-architecture.md), [policy conditions](../features/policies/policy-conditions.md), [roles](../features/roles.md)
