...
The authorizer runs inside the Polaris server. It embeds the Ranger policy engine through the
[Ranger authorization API](../dev/authz-api.md) (`RangerEmbeddedAuthorizer`), downloads the policies of one
...
- An audit store reachable from Polaris if you enable auditing. The Polaris build bundles the Solr audit
  destination and excludes the HDFS destination; see [Audit stores](../services/audit/audit-stores.md).
...
Audit destinations use the [audit framework](../services/audit/index.md) properties with the
`authz.audit.` prefix; see [Audit stores](../services/audit/audit-stores.md) for the other destinations.
...
[Authorization API](../dev/authz-api.md#embedded-authorizer-configuration): `ranger.authz.default.<suffix>`
...
- Operations that the authorizer has no mapping for are denied. The service definition has no
  principal-role or catalog-role resources (an early draft had them; they were removed under RANGER-4910).
...
- Properties of the Polaris entities on the resource path are sent as resource attributes and the
  ...
  [attribute-based policies](../features/abac.md).
...
- Deny items, exceptions, validity schedules, [security zones](../features/sec-zone/intro.md) and
  [tag-based policies](../features/policies/tag-based-policies.md) follow the general
...
evaluation can send the same requests to the [Ranger PDP](../services/pdp/service.md). The PDP configuration
...
`dev-support/ranger-docker/scripts/pdp/ranger-pdp-site.xml`). Ranger's Docker environment does not include a
Polaris container.
...
- [Authorization API](../dev/authz-api.md) — the library the authorizer is built on
