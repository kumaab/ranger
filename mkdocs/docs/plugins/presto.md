...
- The plugin jars from the Ranger build: the plugin archive `ranger-<version>-presto-plugin.tar.gz` contains
  ...
  directory of the coordinator (for example `<presto>/plugin/ranger/`). See [Build](../dev/build.md).
...
so add [audit filters](../services/audit/audit-filters.md) in the service configuration if you want to skip
frequent metadata calls.

## Try it with Docker

`dev-support/ranger-docker` has no Presto compose file; it ships a Trino container instead. Follow
