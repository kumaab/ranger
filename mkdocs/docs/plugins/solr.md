...
authorization field contains one of the user's Ranger roles (see [roles](../features/roles.md)), or, with
`attrs_enabled`, values derived from the user's attributes in the Ranger user store (see
[ABAC](../features/abac.md)). The component is configured in `solrconfig.xml` with the parameters below, read
...
See [audit filters](../services/audit/audit-filters.md).

## Try it with Docker

The Docker environment in `dev-support/ranger-docker` uses Solr both as an audit store and as a
Ranger-protected service. The `ranger-solr` container is defined in
[`docker-compose.ranger-audit-service.yml`](https://github.com/apache/ranger/blob/master/dev-support/ranger-docker/docker-compose.ranger-audit-service.yml)
under the `audit-store-solr` profile, so choose Solr as the audit index store to bring it up:
...
docker compose --profile ${AUDIT_DESTINATIONS} \
  -f docker-compose.ranger.yml -f docker-compose.ranger-audit-service.yml up -d
```

The node enforces the `dev_solr` service (`solr.url=http://ranger-solr.rangernw:8983`), which the environment
creates in Ranger Admin with a starter policy on collection `ranger_audits`. It uses the `security.json` in
[`scripts/solr/solr-security.json`](https://github.com/apache/ranger/blob/master/dev-support/ranger-docker/scripts/solr/solr-security.json),
Kerberos authentication when `KERBEROS_ENABLED=true`, and starts Solr with
`-Dsolr.allowPaths=/etc/ranger,...`. See [Running Ranger with Docker](../getting-started/docker.md).

## Further reading

- [Plugin architecture](../arch/plugin-architecture.md), [policy model](../arch/policy-model.md)
- [Audit stores](../services/audit/audit-stores.md), which covers Solr as the Ranger audit index
