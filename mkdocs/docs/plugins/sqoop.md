...
filters in the service configuration. See [audit filters](../services/audit/audit-filters.md).

## Try it with Docker

There is no Sqoop container in `dev-support/ranger-docker`. To experiment, start Ranger Admin with Docker
(see [Running Ranger with Docker](../getting-started/docker.md)) and point a Sqoop2 server that can reach it
