...
The enforcement point is the Trino coordinator. Trino loads a `SystemAccessControl` implementation named
...
!!! note "Where the code lives"
    ...
    to the Trino project under RANGER-4859); see
...
`dev-support/ranger-docker/docker-compose.ranger-trino.yml` builds the image `ranger-trino` from
`Dockerfile.ranger-trino` (based on `trinodb/trino:${TRINO_VERSION}`, `TRINO_VERSION` in `.env`) and starts
it as container `ranger-trino` on the `rangernw` network. The image copies `scripts/trino/` —
`access-control.properties`, `ranger-trino-security.xml`, `ranger-trino-audit.xml`,
`ranger-policymgr-ssl.xml` and a catalog — into `/etc/trino/`.
