...
- Deny items, exceptions and validity schedules work as described in Policy model.
...
`{USER}`) are not. Change them in the service's `ranger.plugin.audit.filters` configuration;
