...
NodeManager) with the HDFS and YARN plugins active. The ResourceManager web UI is published on port 8088,
and the `dev_yarn` service is created with `yarn.url=http://ranger-hadoop.rangernw:8088`.
...
Ranger service `dev_yarn`, which the plugin in the container enforces.

To verify:

- `docker logs ranger` shows `dev_yarn service created` (or `dev_yarn service already exists` on a restart).
- In Ranger Admin at `http://localhost:6080` (`admin` / `rangerR0cks!`), the service appears in the
  service manager and **Audit → Plugin Status** lists `dev_yarn` once the plugin has downloaded its policies.
- The ResourceManager UI answers at `http://localhost:8088`.
