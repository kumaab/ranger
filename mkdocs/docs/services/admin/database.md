...
JDBC driver jar is not part of Ranger; place it in `ews/webapp/WEB-INF/lib`.

With Docker, the database runs in its own container:

=== "Docker Hub images"

    `apache/ranger-db` (tags `2.4.0` to `2.9.0`) is a PostgreSQL image prepared for Ranger Admin. It is
    started with `POSTGRES_PASSWORD`, `RANGER_DB_USER` and `RANGER_DB_PASSWORD`, and the `apache/ranger`
    container is given the same three variables:
    ...
    The complete sequence is in [Run with Docker](installation.md#run-with-docker).

=== "Build from source (dev-support/ranger-docker)"

    `docker-compose.ranger-db.yml` defines PostgreSQL, MySQL (MariaDB) and Oracle containers;
    `docker-compose.ranger.yml` selects one of them as the `ranger-db` service with `RANGER_DB_TYPE`, and
    the Admin image built from `Dockerfile.ranger` includes the JDBC drivers for these three.
    ...
    The matching Admin configuration is `scripts/admin/configs/ranger-admin-site-${RANGER_DB_TYPE}.yaml`,
    and the password of the database user comes from `RANGER_ADMIN_DB_PASSWORD` in `.env`.
...
The Ranger distribution ships the tooling that performs these stages before Admin starts. In the
`dev-support/ranger-docker` setup `scripts/admin/dba.py` performs them on every start of the `ranger`
container: it imports the core schema when `x_db_version_h` has no `CORE_DB_SCHEMA` entry, applies only
the SQL and Java patches that are not yet recorded, and records the same `x_db_version_h` entries. Admin
itself does not create or alter tables while serving requests.
...
| `inst_at`, `inst_by` | When the patch was applied and by which Ranger version (`Ranger <version>`) |
| `updated_by` | Host that applied the patch |
...
- Policies alone can also be exported and imported as JSON from the UI or REST; That is a logical backup of policies, not of
