...
with the Docker images and compose files from `dev-support/ranger-docker`, and with the service script
`ranger-admin-services.sh`. The second half is a reference for `ranger-admin-site.xml`, grouped by topic.
...
build and start Ranger Admin together with everything it depends on.
...
`ranger-db`
:   The policy database. The flavor is selected with `RANGER_DB_TYPE` (`postgres`, `mysql` or `oracle`)
    from `docker-compose.ranger-db.yml`.

`ranger-zk`
:   ZooKeeper, shared by the Solr audit store and by component containers such as Kafka, HBase and Hive.

`ranger-kdc`
:   A Kerberos KDC, used when `KERBEROS_ENABLED=true`.

The audit store is added with `docker-compose.ranger-audit-service.yml` and a compose profile:
`audit-store-opensearch` (default) or `audit-store-solr`.
...
`rangerR0cks!`. On its first start the container prepares the database schema, starts Admin with
`ranger-admin-services.sh` and creates a set of sample services (`create-ranger-services.py`).
...
| `RANGER_DB_TYPE` | (none) | Database flavor: `postgres`, `mysql` or `oracle`. Not set in `.env`; export it before running compose. |
| `AUDIT_INDEX_STORE` | `opensearch` | Audit store Admin reads from: `opensearch` or `solr`. Must match the compose profile. |
...
`/etc/keytabs`. Building the images, the `ranger_in_docker` wrapper, Docker Hub images and the other
compose files are covered in [Running Ranger with Docker](../../getting-started/docker.md).

!!! warning
    The compose environment uses fixed passwords and a self-contained KDC. It is meant for development
