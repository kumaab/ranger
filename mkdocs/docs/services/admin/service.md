...
[plugins](../../plugins/index.md) that run inside protected services such as Apache Polaris, Trino, Ozone,
...
- **REST layer** (`org.apache.ranger.rest.*`) implements the API under `/service/...`; the UI is only a
  client of that API. See the [REST API overview](../../dev/rest-api.md).
...
The database schema creates four portal users. In the Docker environment they all have the password
...
- To try Ranger quickly, run the Docker environment described in [Running Ranger with Docker](../../getting-started/docker.md);
  it starts Admin together with a database and an audit store.
- For a real deployment, follow [Deployment and configuration](installation.md) and then walk through the
  [first policy tutorial](../../getting-started/first-policy.md).
- If you are integrating an application, read the [architecture overview](../../arch/architecture.md) and
  the [plugin architecture](../../arch/plugin-architecture.md) pages first.
