...
- **Add a service** with the `+` on a card. The form asks for a name, display name, description, an
  ...
  *audit filters* table, and the type-specific *config properties* that come from the service definition
  ...
  [Plugins](../../plugins/index.md) lists these properties.
...
- **Export** and **Import** buttons on the Service Manager exchange policies as JSON, for all services or
  a selection; see [Import and export](../../features/import-export.md). The REST equivalents are
...
are referenced from audit events. Concepts are explained in [Resource policies](../../features/policies/resource-policies.md),
[Tag-based policies](../../features/policies/tag-based-policies.md),
[Row filter and column masking](../../features/policies/row-filter-column-masking.md) and
...
not user roles; see [Roles](../../features/roles.md). The user role (Admin, Auditor, …) is set on the user
...
- [REST API overview](../../dev/rest-api.md)
