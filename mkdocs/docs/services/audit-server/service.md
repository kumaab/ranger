...
The Audit Server lives in the `audit-server/` module and is new in Ranger 3.0. Plugins that write directly to
an audit store continue to work exactly as before; see [Audit framework](../audit/index.md).
...
- The distributions `ranger-<version>-audit-ingestor.tar.gz` and `ranger-<version>-audit-dispatcher.tar.gz`
  produced by the Ranger build, or the Docker images.
...
=== "Docker"
    ...
    `xasecure.audit.destination.auditserver=true`. See [Running Ranger with Docker](../../getting-started/docker.md).
