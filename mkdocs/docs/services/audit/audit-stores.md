...
| --- | --- | --- |
| Solr (SolrCloud) | Yes | Searchable store; the default `ranger.audit.source.type`, used by the released Docker Hub images. |
| OpenSearch | Yes | Searchable store; default audit index of the `dev-support/ranger-docker` setup on master. |
...
With Docker, Solr is available in both setups:

=== "Docker Hub images"

    `apache/ranger-solr` (tags `2.4.0` to `2.9.0`) contains the `ranger_audits` configset. The official
    quick start runs it standalone and creates the core with `solr-precreate`; `apache/ranger-zk` (latest
    tag `2.8.0`) is the ZooKeeper image used when Solr runs in SolrCloud mode.
    ...
    The complete sequence is in [Run with Docker](../admin/installation.md#run-with-docker).

=== "Build from source (dev-support/ranger-docker)"

    Solr is selected instead of the default OpenSearch with `AUDIT_INDEX_STORE=solr`; the
    `audit-store-solr` profile runs a `ranger-solr` container with the `ranger_audits` configset from
    `scripts/solr/solr-ranger_audits`, protected by the Ranger Solr plugin, and the Solr audit dispatcher.
...
OpenSearch is the default audit index of the `dev-support/ranger-docker` setup on master
(`AUDIT_INDEX_STORE=opensearch`, profile `audit-store-opensearch`), where it is fed by the OpenSearch audit
dispatcher; see [Running Ranger with Docker](../../getting-started/docker.md). There is no released Docker Hub
image for this path.
