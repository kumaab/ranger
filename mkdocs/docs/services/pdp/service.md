...
`dataMask` entries. See [Authorization API](../../dev/authz-api.md) for the complete request and response
...
- The PDP distribution, `ranger-<version>-pdp.tar.gz`, produced by the Ranger build
  (see [Building from source](../../dev/build.md)), or the `ranger-pdp` Docker image.
...
=== "Docker"
    ...
    reads `X-Forwarded-User`) and sends audits to the [Audit Server](../audit-server/service.md). See
    [Running Ranger with Docker](../../getting-started/docker.md) for the rest of the stack.
...
(`header`, `jwt`, `kerberos`) and `ranger.authz.remote.pdp.url`; see [Authorization API](../../dev/authz-api.md).
...
any plugin property from [Plugin architecture](../../arch/plugin-architecture.md) can be set this way.
...
To audit through the [Audit Server](../audit-server/service.md), as the docker configuration does:
...
- [Authorization API](../../dev/authz-api.md) — request/response model, `authz-embedded` and `authz-remote`.
